...
$(document).ready(function() {
    var url = 'https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query=New York&forecast_days=6'; //Place your weatherstack API Call Here - access_key to be used: 5bc82451636190abd9d7afe6fe9b20b5
    ...
    $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
        ...
        */      
        $('#heading').html("Today's Weather Forecast - New York");
        $('#local_time').html(data.location.localtime);
        $('#precip_today').html(data.current.precip + "mm");
        $('#humidity_today').html(data.current.humidity + "%");
        $('#wind_today').html(data.current.wind_speed + " mph");
        $('#summary_today').html(data.current.weather_descriptions);
        $('#image_today').attr("src", data.current.weather_icons[0]);
        var temperature_F = getFarenheitTemp(data.current.temperature);
        $('#temp_today').html(temperature_F + " F");
        if (temperature_F > 85) {
            document.getElementById("thermometer_inner").style.backgroundColor = "#FF0000";
        }
        else if (temperature_F < 65) {
            document.getElementById("thermometer_inner").style.backgroundColor = "#0000ff";
        }
        else {
            document.getElementById("thermometer_inner").style.backgroundColor = "#D3D3D3";
        }
    ...
    var forecast_first = getKey(1);
    document.getElementById("5_day_forecast").innerHTML += '<div style="width: 20%;"><div class="card"><div class="card-body"><h5 class="card-title">' + dayOfWeek + '</h5><p class="card-text">High: ' + getFarenheitTemp(forecast_first.maxtemp) +' F'+ '<br>Low: ' + getFarenheitTemp(forecast_first.mintemp) +' F'+ '<br>Sunrise: ' + forecast_first.astro.sunrise + '<br>Sunset: ' + forecast_first.astro.sunset + '</p></div></div></div>';
    var forecast_second = getKey(2);
    document.getElementById("5_day_forecast").innerHTML += '<div style="width: 20%;"><div class="card"><div class="card-body"><h5 class="card-title">' + dayOfWeek + '</h5><p class="card-text">High: ' + getFarenheitTemp(forecast_second.maxtemp) + ' F' + '<br>Low: ' + getFarenheitTemp(forecast_second.mintemp) + ' F' + '<br>Sunrise: ' + forecast_second.astro.sunrise + '<br>Sunset: ' + forecast_second.astro.sunset + '</p></div></div></div>';
    var forecast_third = getKey(3);
    document.getElementById("5_day_forecast").innerHTML += '<div style="width: 20%;"><div class="card"><div class="card-body"><h5 class="card-title">' + dayOfWeek + '</h5><p class="card-text">High: ' + getFarenheitTemp(forecast_third.maxtemp) + ' F' + '<br>Low: ' + getFarenheitTemp(forecast_third.mintemp) + ' F' + '<br>Sunrise: ' + forecast_third.astro.sunrise + '<br>Sunset: ' + forecast_third.astro.sunset + '</p></div></div></div>';
    var forecast_forth = getKey(4);
    document.getElementById("5_day_forecast").innerHTML += '<div style="width: 20%;"><div class="card"><div class="card-body"><h5 class="card-title">' + dayOfWeek + '</h5><p class="card-text">High: ' + getFarenheitTemp(forecast_forth.maxtemp) + ' F' + '<br>Low: ' + getFarenheitTemp(forecast_forth.mintemp) + ' F' + '<br>Sunrise: ' + forecast_forth.astro.sunrise + '<br>Sunset: ' + forecast_forth.astro.sunset + '</p></div></div></div>';
    var forecast_fifth = getKey(5);
    document.getElementById("5_day_forecast").innerHTML += '<div style="width: 20%;"><div class="card"><div class="card-body"><h5 class="card-title">' + dayOfWeek + '</h5><p class="card-text">High: ' + getFarenheitTemp(forecast_fifth.maxtemp) + ' F' + '<br>Low: ' + getFarenheitTemp(forecast_fifth.mintemp) + ' F' + '<br>Sunrise: ' + forecast_fifth.astro.sunrise + '<br>Sunset: ' + forecast_fifth.astro.sunset + '</p></div></div></div>';
    /*
