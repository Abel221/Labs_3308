...
function changeColor(color) {
    var home_background = document.getElementById("home_container");
    home_background.style.backgroundColor = color;
}
...
function loadStatsPage() {
    ...
    for (var row_counter = 2; row_counter < table.rows.length; row_counter++) {
        cell3_value = table.rows[row_counter].cells[2].innerHTML;//Read in a cells current value
        cell4_value = table.rows[row_counter].cells[3].innerHTML;//Read in a cells current value
        if (parseInt(cell3_value) > parseInt(cell4_value)) {
    ...
    document.getElementById("wins").innerHTML = cuWin.toString();
    document.getElementById("losses").innerHTML = otherWin.toString();
}
/*
...
function loadPlayersPage() {
    var dropdownElement = document.getElementById("player_selector");
    for (i = 0; i < players.length; i++) {
        var a = document.createElement("a");
        a.setAttribute("class", "dropdown-item");
        a.href = "#";
        //a.setAttribute("onclick", "switchPlayers(${this.i})");
        if(i == 0)
            a.onclick = function () { switchPlayers(0) };
        if (i == 1)
            a.onclick = function () { switchPlayers(1) };
        if (i == 2)
            a.onclick = function () { switchPlayers(2) };
        if (i == 3)
            a.onclick = function () { switchPlayers(3) };
